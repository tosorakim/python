...
# 실행할 select 문 구성
sql = """
select channel, sum(day1_retention)
from gagong
group by channel;
...
df.columns = ['channel', 'day1_retention']   #  컬럼명을 만들어 줍니다. 
print(df[['channel','day1_retention']])


# 시각화 
plt.bar(df['channel'], df['day1_retention'], color=['#A3E4D7','#48C9B0'])
plt.xlabel('Channel',fontsize=10)
plt.ylabel('day1_retention',fontsize=10)
